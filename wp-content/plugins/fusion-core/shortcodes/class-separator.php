...
class FusionSC_Separator {
	...
	function render( $args, $content = '') {
		...
		$defaults = FusionCore_Plugin::set_shortcode_defaults(
			array(
				...
				'alignment'			=> 'center',
				'bottom_margin'		=> '',
				'border_size'		=> $smof_data['separator_border_size'],
				'icon'				=> '',
	...
	function attr() {
		...
		$attr['style'] .= sprintf( 'margin-top:%s;', Avada_Sanitize::get_value_with_unit( self::$args['top_margin'] ) );		
	
		if( self::$args['bottom_margin'] ) {
			$attr['style'] .= sprintf( 'margin-bottom:%s;', Avada_Sanitize::get_value_with_unit( self::$args['bottom_margin'] ) );
		}
