...
jQuery(document).ready(function($){
	...
	if ( jQuery( 'a[href="themes.php?page=optionsframework"]' ).hasClass( 'current' ) ) {
	...
	$.fn.center = function () {
		this.animate({"top":( $(window).height() - this.height() ) / 2+$(window).scrollTop() + "px"},100);
		this.css("left", ( $( '#of_container' ).width() - $( '#of-nav' ).width() - this.width() ) / 2 + 160 + 'px' );
		return this;
	}
	...
	$('#of_save').live('click',function() {
		...
		$.post(ajaxurl, data, function(response) {
			...
			if (response==1) {
				...
				fail.find( '.of-fail-msg' ).html( response );
				fail.center();
				fail.fadeIn();
			}
...
jQuery(document).ready(function($) {
	...
	brown['counter_filled_color']='#ab8b65';
	...
	$('#section-header_layout img').click(function(e) {
		...
		var selected_val = $('select#header_position').val();

		$('#sec_menu_lh').find('option').removeAttr('selected');

		if(selected_val == 'Top') {
			$('#nav_height').val('83');
			$('#sec_menu_lh').val('44');
			$('#sec_menu_lh').find('option[value=44]').prop('selected', true);
			$('#sec_menu_lh').parent().find('span').text('44');
			$('#padding_header_left, #padding_header_right').val('0px');
			...
			$('#nav_height').val('40');
			$('#sec_menu_lh').val('20');
			$('#sec_menu_lh').find('option[value=20]').prop('selected', true);
			$('#sec_menu_lh').parent().find('span').text('20');
			$('#margin_logo_top, #margin_logo_bottom').attr('value', '31px');
			$('#padding_header_left, #padding_header_right').val('60px');
	...
	$('select#header_position').change(function() {
		...
		if(selected_val == 'Top') {
			...
			var name = $('#section-header_layout img.of-radio-img-selected').parent().find('input[type=radio]').attr('value');
	...
	// Fusion Sorter
	$('.fusion-sortable').each(function() {
		var array = [];
		$(this).find('li').each(function() {
			array.push($(this).attr('id'));
		});
		$(this).closest('.section-fusion_sorter').find('.fusion-sorter-positions').val(array);
	});
	$('.fusion-sortable' ).sortable({
		axis: 'y',
		cursor: 'move',
	...
	$('.section-accordion').toggle(function(e) {
		...
		if( $(this).parents('.accordion').find('#section-header_position').length >= 1 ) {
		...
		if ( ! $(this).parents('.accordion').find( 'input[name=typography_responsive]' ).is( ':checked' ) ) {
			$(this).parents('.accordion').find( '#section-typography_sensitivity, #section-typography_factor' ).hide();
		}
	}, function(e) {
		e.preventDefault();
	...
	window.header_top_v1 = function() {
		jQuery('#section-header_layout, #section-header_shadow, #section-header_100_width, #section-slider_position').show();
		jQuery('#section-side_header_width, #section-header_left_content, #section-header_right_content, #section-header_v4_content, #section-header_banner_code, #section-header_number, #section-header_email, #section-header_tagline').hide();
	}
	window.header_top_v2 = function() {
		jQuery('#section-header_layout, #section-header_shadow, #section-header_100_width, #section-slider_position, #section-header_left_content, #section-header_right_content, #section-header_number, #section-header_email').show();
		jQuery('#section-side_header_width, #section-header_v4_content, #section-header_banner_code, #section-header_tagline').hide();
	}
	window.header_top_v3 = function() {
		window['header_top_v2']();
	}
	window.header_top_v4 = function() {
		jQuery('#section-header_layout, #section-header_shadow, #section-header_100_width, #section-slider_position, #section-header_left_content, #section-header_right_content, #section-header_number, #section-header_email, #section-header_v4_content, #section-header_banner_code, #section-header_tagline').show();
		jQuery('#section-side_header_width').hide();
	}
	window.header_top_v5 = function() {
		window['header_top_v2']();
	}
	window.header_left = function() {
		jQuery('#section-side_header_width, #section-header_shadow, #section-header_left_content, #section-header_right_content, #section-header_number, #section-header_email, #section-header_v4_content, #section-header_banner_code, #section-header_tagline').show();
		jQuery('#section-header_100_width, #section-slider_position, #section-header_layout').hide();
	}
	window.header_right = function() {
		window['header_left']();
	}

	var change_header_options = function( header_position, header_layout ) {
		var header_position = header_position.toLowerCase();
		var header_layout = '_' + header_layout;

		if( header_position != 'top' ) {
			header_layout = '';
		}

		window['header_' + header_position + header_layout]();
	...
	jQuery('#section-header_layout img').click(function() {
		change_header_options( jQuery('#section-header_position #header_position').val(), jQuery(this).parent().find('input[type=radio]').attr('value') );
	});
	...
	jQuery( '#section-' + jQuery( 'input[name=footer_special_effects]:checked' ).val() + '_info' ).show();

	if ( jQuery( 'input[name=footer_special_effects]:checked' ).val() != 'footer_sticky' &&
		 jQuery( 'input[name=footer_special_effects]:checked' ).val() != 'footer_sticky_with_parallax_bg_image'
	) {
		jQuery( '#section-footer_sticky_height' ).hide();
	}

	jQuery( 'input[name=footer_special_effects]:radio' ).change(
		function() {
			...
			if ( jQuery( 'input[name=footer_special_effects]:checked' ).val() == 'footer_sticky' ||
				...
				jQuery( '#section-footer_sticky_height' ).show();
			} else {
				jQuery( '#section-footer_sticky_height' ).hide();
			}
		}
